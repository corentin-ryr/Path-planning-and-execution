...
namespace PathCreation
{
    ...
    public class VertexPath
    {
        ...
        public float GetCurvature(float t, EndOfPathInstruction endOfPathInstruction = EndOfPathInstruction.Loop)
        {
            var data = CalculatePercentOnPathData(t, endOfPathInstruction);
            Vector3 p1 = localPoints[data.nextIndex];
            Vector3 p2 = localPoints[data.previousIndex];
            Vector3 p3;

            if (data.percentBetweenIndices == 0)
            {
                p2 = localPoints[data.nextIndex + 1];
                p3 = localPoints[data.nextIndex + 2];
            }
            else if (data.percentBetweenIndices > 0.5)
            {
                p3 = data.nextIndex + 1 < localPoints.Length ? localPoints[data.nextIndex + 1] : localPoints[data.previousIndex - 1];
            }
            else
            {
                p3 = data.previousIndex - 1 >= 0 ? localPoints[data.previousIndex - 1] : localPoints[data.nextIndex + 1];
            }

            Vector3 V = Vector3.Cross(p1 - p2, p1 - p3);
            float area = V.magnitude * 0.5f;

            return area * 4 / ((p1 - p2).magnitude * (p2 - p3).magnitude * (p3 - p1).magnitude);
        }
